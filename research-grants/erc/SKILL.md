---
name: ors-research-grants-erc
display_name: "ERC Starting/Consolidator/Advanced Grants"
...
tags: [erc, european-research-council, starting-grant, consolidator-grant, advanced-grant, host-institution]
difficulty: advanced
prerequisites:
  - tools: [ERC application portal (Submission system), eSubmission]
  - skills: []
sources_consulted:
  - "ERC: Work Programme and Information for Applicants (current call)"
  - "ERC: Evaluation panels and evaluation criteria"
  - "ERC: Host Institution requirements and support letters"
  - "Improvisions: Explicit B1/B2 separation, Track Record emphasis, ethics issues table structure"
...
- A list of the panel (PE, SH, LS, EN, PE9, etc.) and subdomain the proposal maps to. Use the [ERC evaluation panel list](https://erc.europa.eu/evaluating-grant-applications).

## Core workflow

1. **Pick the ERC grant type** based on the candidate's PhD date and track record. StG is for early-career, CoG is for mid-career, AdG is for established. The budget reflects the career stage and evidence requirements.

2. **Pick the evaluation panel** from the ERC's list. ERC panels are SH (Social Sciences and Humanities), LS (Life Sciences), PE (Physical Sciences and Engineering), PE9 (Computational Sciences). Use the correct subdomain. A proposal that maps to the wrong panel is less damaging than one that is split across panels.

3. **Confirm the host institution's ERC agreement status.** An institution in an EU or associated country with a signed ERC agreement can host. The Host Institution is the applicant's employer and signs the Host Institution Commitment Letter.

4. **Draft the B1 Extended Synopsis (2 pages) first, as a standalone document.** This is the ERC's most-distinctive element. It must give the reviewer the "what, why, how, and who" in two pages without referring to the B2. It is not a shorter version of the B2; it is a separate abstract. Reviewers read the B1 first, and the B1 gates the B2.

5. **Draft the B2 Full Proposal (≈7–8 pages) with the four sections the ERC demands:**
...
6. **Write the CV and Track Record** — the ERC's unique contribution. The CV is not an NIH/NSF biosketch; it is a publication-and-output list weighted toward the most recent 5 years. The Track Record is the "5 best papers" or equivalent outputs, with a narrative that connects each output to the ERC line of research. The ERC wants the PI to identify which work makes them the right person for this project.

7. **Draft the ethics issues table.** Complete all applicable fields (Human Participants, Animal Welfare, Privacy / Data, Dual Use, Environmental Impact, Other). A "no ethics issues" answer requires a justification, not silence.

8. **Draft the short data management plan** in the ERC template, consistent with the B2's data description.

9. **Obtain the Host Institution Commitment Letter** — signed by the host institution's legal representative. The letter confirms the institution's commitment to hosting the PI for the grant duration, provides the research infrastructure, and confirms the PI's employment conditions. The host institution is the PI's employer.

10. **Assemble and submit via the ERC application portal.** The ERC uses eSubmission, not ASSIST or Research.gov. Verify the portal and the submission window. Plan submission at least 48 hours before the deadline.
...
- ERC: [Apply for a grant](https://erc.europa.eu/apply-grant) — current Work Programme, call calendar, and application portal.
- ERC: [Work Programme and Information for Applicants](https://erc.europa.eu/apply-grant) — the canonical document.
- ERC: [Evaluation panels](https://erc.europa.eu/evaluating-grant-applications) — panel list, criteria, and evaluation process.
- ERC: [Ethical and linguistic checks](https://erc.europa.eu/apply-grant) — ethics issues table guidance.
- ERC: [Host Institution requirements](https://erc.europa.eu/apply-grant/host-institution) — institutional eligibility and commitment letter.
- European Commission: [Funding & Tenders Portal](https://ec.europa.eu/info/funding-tenders) — the central EU portal where ERC calls are published.
